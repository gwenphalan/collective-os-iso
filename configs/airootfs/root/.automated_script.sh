...
install_base_system() {
  ...
  # Ensure limine-snapper-sync (and its snapper/alpm hooks) are not active in the live env
  # so Archinstall's snapper create-config doesn't trip over missing Limine config.
  if pacman -Qi limine-snapper-sync >/dev/null 2>&1; then
    pacman -R --noconfirm limine-snapper-sync || true
  fi
  rm -f /usr/lib/snapper/plugins/10-limine-snapper-sync \
        /usr/share/libalpm/hooks/90-limine-snapper-sync.hook \
        /usr/share/libalpm/scripts/limine-snapper-sync 2>/dev/null || true
  ...
  if [[ -f /etc/pacman-offline.conf ]]; then
    PACMAN_CONF=/etc/pacman-offline.conf \
      archinstall \
